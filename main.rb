...
class MathExercises
  def conditions(number: ) # puts task - conditions
    $number = number
    CONDITIONS[number] # Const from file conditions.rb
  end

  def line(a, b) # line is used to solve task25
    Math.sqrt((b[0] - a[0])**2 + (b[1] - a[1])**2)
  end

  def factorial(a) # factorial is used to solve task235
    (1..a).inject(:*) || 1
  end

  def gcd(a, b) #gcd is used to solve task323
    if a % b == 0
      b
    else
      gcd(b, a % b)
